...
1. **Connector inventory.** List available MCP tools/connectors at the start of the run (explicit user requirement). Bind the domain playbooks' data capabilities to whatever tools are armed this session (deferred tools must be surfaced via `ToolSearch`; a capability with no matching tool falls back to web search) and determine the Telegram delivery path: direct Bot API (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in the environment) is primary, Composio's Telegram send tool is the fallback if those env vars are absent or a direct call errors. Only if **neither** path is available does the run gate: run the Composio authenticate flow, surface the auth link to the user, and **stop the run** — do not proceed to data gathering. A run that cannot deliver produces stale analysis by the time auth completes; simplest correct behavior is to gate the whole run on delivery capability, then rerun once connected.
...
5. **Telegram delivery.** Send the message sequence defined in DESIGN.md, in order, each within the 4096-char Telegram limit, using the delivery mechanism confirmed in stage 1 (direct Bot API, or Composio on fallback).
...
- `.env` is gitignored (secrets never committed) and holds `FRED_API_KEY` plus `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` for direct Telegram delivery. `data/` is **committed to the repo** — the routine runs daily and each run's snapshots, summary, and catalysts are persisted as history (see CLAUDE.md Stage 2's commit step).
...
- `package.json`, `tsconfig.json`, `.env.example` (documents `FRED_API_KEY`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, no real values), `.gitignore`.
...
- **Telegram unavailable (both paths)**: no `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in the environment AND no Composio Telegram tool found — run the Composio authenticate action (found in the live inventory — exact name varies), surface the auth link as plain text to the user (not to Telegram, since Telegram isn't connected yet), and stop the entire run before data gathering begins (see stage 1). Do not loop retrying the complete-authentication action — it needs a user-supplied code/redirect, so one attempt then stop-and-report is correct.
